...
def normalise_input(user_input):
    ...
    user_input = remove_spaces(user_input).lower()
    return user_input
...
def print_menu(exits):
    ...
    print("You can:")
    for x in exits:
        print_menu_line(x,exit_leads_to(exits,x))
...
def menu(exits):
    ...
    while True:
        ...
        chosen = input()
        # Normalise the input
        chosen = normalise_input(chosen)
        # Check if the input makes sense (is valid exit)
            # If so, return the player's choice
        if not is_valid_exit(exits,chosen):
            print("Please type a correct exit")
            continue
        else:
            return chosen
            break
